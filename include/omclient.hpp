...
class OMClient_EXPORT OMClient: public QObject
{
    Q_OBJECT
#if defined(USE_QML)
    Q_PROPERTY(QJsonObject  currentWeather READ currentWeather NOTIFY currentWeatherChanged)
    Q_PROPERTY(float  latitude READ latitude WRITE setLatitude  NOTIFY positionChanged)
    Q_PROPERTY(float  longitude READ longitude WRITE setLongitude NOTIFY positionChanged)
...
private:
    OMResponse*  getReplyRest(const QString& path, const QString &query=QString())const;
    QUrl m_apiUrl;
